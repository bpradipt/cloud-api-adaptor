...
// Default properties for Docker
var defaultDockerProperties = map[string]string{
...
func initDockerProperties(properties map[string]string) error {
	...
	for k, v := range defaultDockerProperties {
		if prop, ok := properties[k]; !ok || prop == "" {
			properties[k] = v
		}
	}
...
func createKindCluster(workingDir string) error {
	...
	cmd.Env = append(cmd.Env, "CONTAINER_RUNTIME="+DockerProps.ContainerRuntime)
