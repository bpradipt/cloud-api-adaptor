...
func (l *DockerProvisioner) CreateCluster(ctx context.Context, cfg *envconf.Config) error {
	...
	// Install docker runtime on the cluster node
	if err := installDockerRuntime(l.wd); err != nil {
		log.Errorf("Error installing Docker runtime: %v", err)
		return err
	}
...
func (l *DockerProvisioner) DeleteCluster(ctx context.Context, cfg *envconf.Config) error {
	...
	err = uninstallDockerRuntime(l.wd)
	if err != nil {
		log.Errorf("Error uninstalling Docker runtime: %v", err)
		return err
	}
...
func installDockerRuntime(workingDir string) error {
	// Install docker runtime by executing the script on the cluster node
	cmd := exec.Command("/bin/bash", "-c", "./install_docker.sh install")
	cmd.Dir = workingDir
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	err := cmd.Run()
...
func uninstallDockerRuntime(workingDir string) error {
	// Uninstall docker runtime by executing the script on the cluster node
	cmd := exec.Command("/bin/bash", "-c", "./install_docker.sh uninstall")
	cmd.Dir = workingDir
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	err := cmd.Run()
