...
func SelectInstanceTypeToUse(spec InstanceTypeSpec, specList []InstanceTypeSpec, validInstanceTypes []string, defaultInstanceType string) (string, error) {
	...
	if spec.VCPUs != 0 && spec.Memory != 0 {
		instanceType, err = GetBestFitInstanceType(specList, spec.Memory, spec.VCPUs)
		if err != nil {
	...
	instanceType, err = VerifyCloudInstanceType(spec.InstanceType, validInstanceTypes, defaultInstanceType)
	if err != nil {
