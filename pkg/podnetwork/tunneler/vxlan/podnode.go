...
func (t *podNodeTunneler) Setup(nsPath string, podNodeIPs []net.IP, config *tunneler.Config) error {
	...
	if err := podNS.SetHardwareAddr(podVxlanInterface, config.PodAddr); err != nil {
		return fmt.Errorf("failed to set pod Mac %s on %s: %w", config.PodAddr, podVxlanInterface, err)
	}
